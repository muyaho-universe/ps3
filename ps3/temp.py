...
def unwrap_const(expr):
    """
    If expr is a pyvex.expr.Const whose .con is itself an IRExpr/SymbolicValue,
    peel it off so downstream comparison sees the real node.
    """
    while isinstance(expr, Const) and isinstance(expr.con, (SymbolicValue, IRExpr)):
        expr = expr.con
    return expr
...
# print("put_general == put_wrong:", per_related(put_general, put_wrong)) # True
# print("put_wrap_t == put_wrong:", per_related(put_wrap_t, put_wrong)) # True
# print("put_wrap_t === put_general:", per_related(put_wrap_t, put_general)) # True
# print("put_wrap_t === put_concrete:", per_related(put_wrap_t, put_concrete)) # False

# assert per_related(put_general, put_concrete)   # ✔︎ True
